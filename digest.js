...
window._isUntouched = function(lead) {
  ...
  // Consistent with what's displayed for this section (see
  // mkGroupedUntouchedSection): "untouched" is measured by the follow-up
  // date alone, so a lead can never show up here with a days-ago number
  // under 7 — the inclusion test and the displayed number are now the
  // same calculation.
  ...
  // Untouched if no actions AND no follow-up for 7+ days
  return !hasActions && daysSinceFollowUp >= 7;
};
  ...
  const mkGroupedUntouchedSection = (icon, title, color, groupedLeads, accent) => {
    ...
    tabOrder.forEach(tab => {
      ...
      leads.forEach(l => {
        ...
        // Days-ago always matches the "Last Follow-up" date shown right next
        // to it — previously this preferred days-since-last-email when
        // email history existed, which meant two leads with the exact same
        // follow-up date could show wildly different "Xd ago" numbers
        // depending on unrelated email activity. That's fixed: this now
        // always reflects the visible follow-up date, consistently.
