...
(function () {
  ...
  function enableOnTable(wrap) {
    ...
    headers.forEach((th, idx) => {
      ...
      if (!th.querySelector('.cr-handle')) {
        ...
        // For the last column, position the handle fully INSIDE the cell.
        // Otherwise, the wrap's overflow:hidden / overflow:auto clips the
        // handle and the user can't grab it.
        if (idx === headers.length - 1) handle.classList.add('cr-handle-last');
  ...
  // Autofit every column in a tab to its content — the browser's own
  // table layout engine measures each column's real natural width
  // (accounting for badges, dropdowns, checkboxes, icons — not just
  // plain text), then we lock those widths in as fixed pixel values,
  // same as a manual drag, so they persist exactly the same way.
  window.autofitColumns = function (tab) {
    if (!tab) return;
    const wrap = tab === 'intake'
      ? document.getElementById('intake-table-wrap')
      : document.getElementById('leadtable-wrap-' + tab);
    ...
    // Temporarily release the fixed widths so the browser can lay the
    // table out naturally and tell us how wide each column actually
    // wants to be. This all happens synchronously in one JS task, so
    // nothing gets painted in between — no visible flicker.
    ...
    // Force a layout pass so getBoundingClientRect() below reflects the
    // natural (un-fixed) widths, not stale cached values.
    ...
    headers.forEach((th) => {
      const key = th.getAttribute('data-col-key');
      if (!key) return;
      // Small buffer so text doesn't sit flush against the resize handle.
      const natural = Math.ceil(th.getBoundingClientRect().width) + 6;
      map[key] = Math.max(MIN_W, Math.min(MAX_W, natural));
    });

    // Snap back to fixed layout with the computed widths locked in.
    table.style.tableLayout = prevTableLayout || 'fixed';
    ...
    saveWidths(tab, map);
    if (window.showSuccess) window.showSuccess('Columns autofit', 'Widths adjusted to fit content — nothing should be cut off now.');
  };
  ...
  const css = `
    ...
    /* Last column's handle: sit fully INSIDE the cell so the table-wrap's
       overflow doesn't clip it (which would make it ungrabbable). */
    th .cr-handle.cr-handle-last {
      right: 0;
      width: 6px;
    }
    th .cr-handle.cr-handle-last::before {
      left: 2px;
    }
