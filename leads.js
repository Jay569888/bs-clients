...
// ── Row highlight source of truth ──────────────────────────
// Row color is driven entirely by the Level field's current value —
// not by whether/what email was sent. Call this any time lead.level
// changes, from ANY code path (manual dropdown selection, or an
// automated sequence step that sets lead.level programmatically),
// so the row's color always matches what Level currently shows.
//   "For Review"   -> green
//   "5-Day Notice" -> red
//   anything else  -> no highlight
window._syncRowAlertFromLevel = function(lead) {
  if (!lead) return;
  const lvl = String(lead.level || '').trim();
  if (lvl === 'For Review') {
    lead.rowAlert = 'review';
    // Stamp the moment this lead became For Review, but only once per
    // "becoming" — if it's already For Review, keep the original time so
    // re-saving the same lead doesn't bump it to the back of the queue.
    if (!lead.forReviewAt) lead.forReviewAt = new Date().toISOString();
  } else if (lvl === '5-Day Notice') {
    lead.rowAlert = 'drop';
    lead.forReviewAt = '';
  } else {
    lead.rowAlert = '';
  ...
  if(!existingWrap){
    ...
    container.innerHTML=alertHtml+`<div class="intake-table-toolbar" style="display:flex;gap:6px;margin-bottom:8px;flex-wrap:wrap;align-items:center;flex-shrink:0">
      ...
      <div style="margin-left:auto;display:flex;gap:4px;flex-wrap:wrap;align-items:center">
        ...
        <button class="btn btn-sm btn-outline" title="Resize every column to fit its content" onclick="window.autofitColumns('intake')">⤢ Autofit</button>
        <button class="btn btn-sm btn-outline" title="Restore default column widths" onclick="window.resetColumnWidths('intake')">↺ Reset Widths</button>
...
window._renderIntakeTable=function(leads){
  ...
  const rows=leads.map(l=>{try{
    ...
    return`<tr id="lead-row-${l.id}" tabindex="0" data-id="${encodeURIComponent(l.id)}" data-tab="intake" style="${rowBg?'background:'+rowBg:''}">
      ...
      <td ondblclick="if(!event.target.closest('.copy-cell'))window.openLeadDrawer('intake','${l.id}')" style="font-weight:600;color:var(--primary);cursor:pointer;${l.nameHighlight?'background:'+l.nameHighlight+';':cellBg}">${starBadge}${l.prioTomorrow?'❤️ ':''}${replyBadge}${l.name?`<span class="copy-cell" data-copy="${l.name.replace(/"/g,'&quot;')}" data-copy-kind="name" title="Double-click to copy">${l.name}</span>`:''}${window._leadRowActionsHtml('intake',l.id)}</td>
  ...
  if(!existingWrap){
    ...
    el.innerHTML=alertHtml+`<div class="page-header" style="flex-shrink:0">
      ...
      <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center">
        ...
        <button class="btn btn-sm btn-outline" title="Resize every column to fit its content" onclick="window.autofitColumns('${tab}')">⤢ Autofit</button>
        <button class="btn btn-sm btn-outline" title="Restore default column widths" onclick="window.resetColumnWidths('${tab}')">↺ Reset Widths</button>
...
window._getDisplayLeads = function(tab) {
  ...
  // ── "For Review" leads are pinned to the top ALWAYS — this happens
  // before any other sort (explicit column sort or the smart stack) so
  // a column sort can never push them down. Among themselves they're
  // ordered oldest-first by when they became "For Review" (forReviewAt),
  // not by whatever column is being sorted. Everything else below is
  // sorted/bucketed exactly as before, among the non-review leads only.
  const isForReview = (l) => String(l.level || '').trim() === 'For Review';
  const forReviewLeads = sorted.filter(l => isForReview(l)).sort((a, b) => {
    const ta = a.forReviewAt ? new Date(a.forReviewAt).getTime() : 0;
    const tb = b.forReviewAt ? new Date(b.forReviewAt).getTime() : 0;
    return ta - tb; // oldest first
  });
  sorted = sorted.filter(l => !isForReview(l));
  ...
  if (ss.col) {
    ...
    // When the user has explicitly sorted, respect that order for the
    // non-review leads — don't re-bucket them. (For Review leads were
    // already pulled out above and always lead the list.)
    return [...forReviewLeads, ...sorted];
  }

  // No explicit sort → smart stack for the non-review leads. Mutually
  // exclusive buckets in precedence order: starred > prioTomorrow >
  // overdue > rest. This guarantees every row appears exactly once (the
  // old code dropped rows that were both starred AND prioTomorrow).
  ...
  return [...forReviewLeads, ...starLeads, ...prioLeads, ...overdueLeads, ...restLeads];
};
...
window.renderLeadTable=function(tab,leads){
  ...
  const rows=leads.map(l=>{try{
    ...
    return`<tr id="lead-row-${l.id}" tabindex="0" data-id="${l.id}" data-tab="${tab}" class="${rowClass}" style="background:${rowBg}">
      ...
      <td class="editable" ondblclick="if(!event.target.closest('.copy-cell'))window.openLeadDrawer('${tab}','${l.id}')" style="font-weight:600;color:var(--primary);cursor:pointer;${l.nameHighlight?'background:'+l.nameHighlight+';':cellBg}">${starBadge}${l.prioTomorrow?'❤️ ':''}${replyBadge}${l.name?`<span class="copy-cell" data-copy="${l.name.replace(/"/g,'&quot;')}" data-copy-kind="name" title="Double-click to copy">${l.name}</span>`:''}${window._leadRowActionsHtml(tab,l.id)}</td>
      <td style="${cellBg}">${phoneHtml}</td>
      <td style="font-size:10px;${cellBg}">${l.email?`<span class="copy-cell" data-copy="${l.email.replace(/"/g,'&quot;')}" data-copy-kind="email" title="Double-click to copy">${l.email}</span>`:''}</td>
...
window._applyNDayNoticeAfterSend = function(lead, tab, templateName, subject) {
  ...
  // CANCELLATION — non-N-day email sent, clear stale drop-date tracking
  // (Row color is no longer touched here — only the Level dropdown sets
  // rowAlert now. This just keeps the _dropDate/notes digest tracking
  ...
  // N-Day Notice email sent — track the drop date for EOD digest reporting.
  // Row color is no longer set here — only the Level dropdown sets rowAlert.
...
window._handleLevelChange = function(tab, leadId, selectEl) {
  ...
  // Row highlighting is derived from the Level value — see
  // window._syncRowAlertFromLevel for the "For Review" / "5-Day Notice" rule.
  const lead = tab === 'intake'
    ? (window.state.leads.intake||[]).find(l => l.id === leadId)
    : window.findLeadById(leadId);
  if (lead) {
    const wasForReview = String(lead.level || '').trim() === 'For Review';
    lead.level = val;
    window._syncRowAlertFromLevel(lead);

    // Auto-log to EOD the moment this lead becomes "For Review" via the
    // dropdown. Merge with whatever's already logged for this lead today
    // (calls/texts/etc.) rather than overwriting it.
    const nowIsForReview = val === 'For Review';
    if (nowIsForReview && !wasForReview) {
      const reviewPhrase = `Requested Ilya to review - ${lead.name || ''}`;
      const existing = window.buildConsolidatedEodText ? window.buildConsolidatedEodText(lead) : '';
      const eodText = (existing && !existing.includes('Requested Ilya to review'))
        ? `${existing}, ${reviewPhrase}`
        : reviewPhrase;
      if (window.upsertTodaysEodEntry) {
        window.upsertTodaysEodEntry({ leadId: lead.id, leadName: lead.name, tab, newText: eodText });
      }
    }
  }
  ...
  // Re-render the whole table (not just this row) so a lead entering or
  // leaving "For Review" visibly jumps to/from the top immediately,
  // instead of only reordering on the next reload.
  if (tab === 'intake') {
    window.renderIntakeList && window.renderIntakeList();
  } else if (window.TABS && window.TABS.includes(tab)) {
    window.renderLeadPage && window.renderLeadPage(tab);
  }
};
...
window.buildConsolidatedEodText = function(lead) {
  ...
  // Intake leads always log one fixed phrase for any activity — not the
  // usual per-channel "Called, Texted" breakdown used in every other tab.
  // This applies no matter which action triggered the EOD log (checkbox
  // toggle, quick-send template, etc.) since they all funnel through here.
  const leadTab = window.findLeadTab ? window.findLeadTab(lead.id) : null;
  if (leadTab === 'intake') {
    const anyActivity = !!(lead.call || lead.vm || lead.text || lead.emailChk || lead.upload);
    return anyActivity ? `Chased Intake of ${name}` : '';
  }
...
(function(){
  ...
  document.addEventListener('dblclick', async (e) => {
    ...
    if (ok) {
      const label = kind === 'phone' ? 'Phone number' : kind === 'email' ? 'Email' : kind === 'name' ? 'Name' : 'Value';
